...
void neon_md5_transform_4(uint8_t blocks[4][MAX_BLOCKS*64], bit32* state, int total_blocks) {
    ...
    // 初始化NEON寄存器
    a = vdupq_n_u32(0x67452301);
    b = vdupq_n_u32(0xefcdab89);
    c = vdupq_n_u32(0x98badcfe);
    d = vdupq_n_u32(0x10325476);
	...
	for (int block_idx = 0; block_idx < total_blocks; block_idx++) {
		...
		for (int i = 0; i < 16; i++) {
            uint32_t words[4];
            for (int j = 0; j < 4; j++) {
                int base = block_idx * 64 + i * 4;
                words[j] = (uint32_t)blocks[j][base] |
                          ((uint32_t)blocks[j][base + 1] << 8) |
                          ((uint32_t)blocks[j][base + 2] << 16) |
                          ((uint32_t)blocks[j][base + 3] << 24);
            }
            x[i] = vld1q_u32(words);
        }
   ...
   // 修改后的结果存储部分
alignas(16) uint32_t result[4][4]; // [4个密码][4个状态字]
vst1q_u32(result[0], a);
vst1q_u32(result[1], b);
vst1q_u32(result[2], c);
vst1q_u32(result[3], d);

// 转置存储到输出state（按密码顺序）
// 转置并调整字节序
for (int i = 0; i < 4; i++) {
	for (int j = 0; j < 4; j++) {
		uint32_t value = result[j][i];
		state[i*4 + j] = ((value & 0xff) << 24) |
						((value & 0xff00) << 8) |
						((value & 0xff0000) >> 8) |
						((value & 0xff000000) >> 24);
	}
	...
	void SIMD_MD5Hash(const vector<string>& passwords, bit32* state) {
		constexpr size_t batch_size = 4;
		alignas(64) uint8_t blocks[batch_size][MAX_BLOCKS * 64]; // 支持多块
		int msg_lengths[batch_size] = {0}; // 存储每个消息的长度
		
		for (size_t i = 0; i < passwords.size(); i += batch_size) {
			size_t current_batch = min(batch_size, passwords.size() - i);
			int max_blocks = 0;
			
			// 预处理所有密码
			for (size_t j = 0; j < current_batch; j++) {
				Byte* padded = StringProcess(passwords[i + j], &msg_lengths[j]);
				int num_blocks = msg_lengths[j] / 64;
				max_blocks = max(max_blocks, num_blocks);
				memcpy(blocks[j], padded, msg_lengths[j]);
				delete[] padded;
			}
			
			// 处理每个批次
			neon_md5_transform_4(blocks, &state[i*4], max_blocks);
		}
	}
/**
